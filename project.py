...
def remove_stop_words(dic):
    with open("stopwords.txt", "r", errors="surrogateescape") as file:
        stopwords = [word.strip().lower() for word in file.readlines()]
        
    updated_dict = {key: value for key, value in dic.items() if not is_stopword(key.lower(), stopwords)}
...
def is_stopword(word, stopwords):
    ...
    # Check if any substring within the word matches a stop word
...
def print_accuracy_for_stop_words():
    ...
    print("\nConclusion: Removing stop words from the dataset does help in improving the efficiency as the no of token were "
      +"  changed from "+str(len(log_prob_word_ham))+" to " + str(len(log_prob_word_ham_sp))+". This also reduces the accuracy as there are less number of words to train upon. This is why we are increasing the dataset size so that the number of words are enough so that the accuracy relatively increases as well")
...
def classify_email_from_gui():
    ...
    # Update result label based on classification
    classification = classify_email(log_prob_word_spam, log_prob_word_ham, prior_spam, prior_ham, email_content)
    
    # Play corresponding video
    if classification == "spam":
        play_video_in_gui("spam.mp4")
    else:
        play_video_in_gui("ham.mp4")
...
def play_video_in_gui(video_path):
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        result_label.config(text="Error: Failed to open video.")
        return
    ...
    while True:
        ...
        # Resize frame to fit in the GUI window
    ...
    # Clear video label after video ends
